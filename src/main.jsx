...
function Cardapio({products,saveProducts,adds,saveAdds}){
 ...
 const persist=async(next)=>{ saveProducts(next); if(supabase){ for(const [idx,item] of next.entries()){ await saveProductRemote(item,idx); } } };
 const addProd=async()=>{ if(!p.name||!p.price) return alert('Preencha nome e preço.'); const item={...p,id:slugify(p.name)+'-'+Date.now(),price:Number(p.price),active:p.active!==false,sort_order:products.length}; const next=[...products,item]; await persist(next); setP(blank); };
